...
public class JwtTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            // BotTokenFilter runs before this filter; if it already authenticated the
            // request (valid X-Bot-Token), the bot principal wins and a JWT must never
            // overwrite it (see spec "Service token (issue #47)" precedence rows).
