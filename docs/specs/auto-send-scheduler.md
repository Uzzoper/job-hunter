...
- **THEN** a `TemplateEmail` is used (company name placeholder, no AI call)
...
- **WHEN** a tick runs
- **THEN** same as Scenario 1 (template email, sent)

### Scenario 3: low-score eligible (personalized AI email)
...
- **THEN** a personalized `EmailDraft` is generated by AI, then sent
- **AND** the draft is marked `SENT`
...
- **Priority order: highest matchScore first.** High-score jobs (template, instant) go out before low-score jobs (AI, slower). Within the same score tier, oldest first.
- **Score threshold for email type:** `matchScore >= 60` → `TemplateEmail` (no AI, company name placeholder only). `matchScore < 60` → personalized `EmailDraft` (AI-generated).
- **`EmailStatus` gains `APPROVED`:** sits between `PENDING` and `SENT`. Only meaningful when `require-review = true`; ignored when `false` (reads `PENDING` directly).
- **Daily cap:** 50 per user per calendar day (UTC). Resets at midnight.
- **Feasibility with local AI (qwen2.5:3b):** Yes. Template emails are instant. Personalized emails take ~30s each. With 120s tick interval and 50/day cap, the scheduler handles both tiers comfortably.
...
public interface AutoSendEligibilityPort {
    // Highest-scoring eligible draft, preferring TemplateEmail-ready ones (>60).
    // Returns empty if daily cap hit or none eligible.
...
void tick() {
    ...
    eligibilityPort.nextEligibleDraft().ifPresent(draft -> {
        try {
            var email = draft.matchScore() >= 60
                ? templateEmailService.generate(draft)
                : emailGenerationService.generate(draft.userId(), draft.jobId());
